...
function get_tini_arch(string $arch, string $variant, array &$errors) {
    if ($arch == "arm") {
        $found = find_table(__DIR__."/arch.arm.txt", $variant);
        if ($found !== false) return $found;
        $errors[] = "arm with $variant is not supported";
    } else {
        $found = find_table(__DIR__."/arch.all.txt", $arch);
        if ($found !== false) return $found;
...
function get_tini_latest_version() {
    $latest_file_time = @filemtime(__DIR__."/latest.txt");
    ...
    if ($latest_file_time === false || $latest_file_time < (time() - (24*60*60))) {
        ...
        file_put_contents(__DIR__."/latest.txt", $latest);
        return $latest;
    } else {
        return file_get_contents(__DIR__."/latest.txt");
    }
}
