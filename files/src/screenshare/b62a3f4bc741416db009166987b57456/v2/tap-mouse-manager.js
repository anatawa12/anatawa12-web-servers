...
function mouseDown(/** "left"|"middle"|"right" */mouse) {
    ...
    sendMsg(new KeyToggle(mouseNumberMap(mouse), KeyToggleFlags.empty.withIsDown(true)))
}

function mouseUp(/** "left"|"middle"|"right" */mouse) {
    addLog("mouseUp: " + mouse);
    sendMsg(new KeyToggle(mouseNumberMap(mouse), KeyToggleFlags.empty.withIsDown(false)))
}
...
function mouseMove(/** number */moveX, /** number */moveZ) {
    ...
    sendMsg(new MoveMouse(Math.floor(moveX / desktopSize), Math.floor(moveZ / desktopSize)));
}

function mouseDrag(/** number */moveX, /** number */moveZ) {
    setMouse(mouseX - moveX / desktopSize, mouseY - moveZ / desktopSize);
    sendMsg(new MoveMouse(Math.floor(mouseX / desktopSize), Math.floor(mouseY / desktopSize)));
}

function mouseScroll(/** number */moveX, /** number */moveZ) {
    //addLog("mouseScroll: " + moveX + ", " + moveZ);
    sendMsg(new MouseScroll(Math.floor(moveX), Math.floor(moveZ)));
}

$("#tap-version").text("2.0.0b1")
