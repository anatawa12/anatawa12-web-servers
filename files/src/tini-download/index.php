...
if (!isset($_GET["arch"])) {
    ...
    <h2>Examples</h2>
    <ul>
        <li>https://files.anatawa12.com/tini-download?arch=x64</li>
        <li>in DockerFile (with <code># syntax=docker.io/docker/dockerfile:1.3.0</code>), <code>ADD --chmod=555 "https://files.anatawa12.com/tini-download?arch=${TARGETARCH}&variant=${TARGETVARIANT}" /tini</code></li>
    </ul>
    <?php
    exit;
}
...
if (!find_line("./avalivals.txt", $suffix)) {
    $errors[] = "The $type package of tini for $tini_arch is not avalval.";
}
handle_errors($errors);
